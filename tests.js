...
let testCases = ['#f4','#f4f4','#f4f4f4','#fff','#000','#0', '#', '', 'f4f4f4', 'f4f'];
testCases.forEach(color => {
    try{
        console.log(hexStringToRgbArray(color));
    }catch(err){
        const message = err.message === 'Invalid input, hexColorStr must be of the form "#fff", "#ffffff" or "#f4f4f4"' ? 'pass':'fail';
        console.log(message);
    }
});
...
testCases = [[224, 201, 123], [223, 200, 123], [255, 255, 255], [0,0,0], [30,30,30]];
